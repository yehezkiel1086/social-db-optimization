-- 1) Cursor-based feed (latest-first) — include followees' posts
-- This query fetches the feed for a user ('alice' in this case).
-- It uses cursor-based pagination to efficiently load more posts.
-- For the first page, we use `now()` as the cursor timestamp.
WITH followees AS (
  ...
  WHERE
    ...
    -- For subsequent pages, you would use the `created_at` and `id` of the last post from the previous page.
...
-- 2) Combined feed with algorithmic ranking (score = recency * weight + interactions)
-- This query ranks posts based on a scoring algorithm to show more relevant content first.
-- The score considers reactions, comments, and the age of the post.
WITH followees AS (
  SELECT followee_id FROM user_follows WHERE follower_id = (SELECT id FROM users WHERE username = 'alice')
), candidates AS (
...
-- 3) Notifications query (unseen recent)
-- Fetches the 50 most recent, unseen notifications for a user ('charlie' in this case).
...
-- 4) Conversation messages (pagination)
-- Fetches a page of messages from a specific conversation using cursor pagination.
-- This example fetches messages from the private chat between 'alice' and 'bob'.
SELECT m.*
FROM messages m
